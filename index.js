 ...
 */
const getArtistByIndex = (array, index) => {
 ...
 */
const removeArtist = (artists, index) => {
  for (let i = 0; i < artists.length; i++) {
    const remove = artists.slice(index);
    return remove;
  }
};
...
const addArtist = (information) => {
  return {
    id: 20,
    name: "Matt",
    years: 1992,
    genre: "Web Design",
    nationality: "white black",
    bio: "blah",
  };
};
...
const lotsOfArt = (artist) => {
  let paintingArr = [];
  for (let i = 0; i < artist.length; i++) {
    if (artist[i].paintings > 100) {
      paintingArr.push(artist[i].name);
    }
  }
  return paintingArr;
};
